...
describe("PatientGenerator", () => {
    ...
    describe("buildPatient", () => {
        it("should return a simple patient element", () => {
            ...
            expect(actual).toEqual(expectedGood);
        });

        it("should throw error when given unhandled gender", () => {
            const outcomeBadGenderCode = Object.assign({}, outcome1);
            outcomeBadGenderCode.genderCode = 23;
            expect(() => {
                const ignoredActual = subject.buildPatient("ignoredPatientId", outcomeBadGenderCode);
            }).toThrow(new Error("Unhandled gender value \"23\"."));
        });
